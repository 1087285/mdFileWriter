...
- **[v1.4.0変更] `initEditor()` `customHTMLRenderer` 削除**: `customHTMLRenderer` は WYSIWYG ProseMirror 描画に介入しないことが Windows 実機で判明したため削除。
- **[v1.4.0変更] `preprocessMarkdown()` 変換方式変更**: `/ {2,}(\n)/g` → `'  $1'`（スペース正規化）から `/ {2,}\n/g` → `'<br>\n'`（inline HTML 変換）に変更。`setMarkdown()` が受け付けた `<br>` を ProseMirror が `hard_break` ノードに変換し、WYSIWYG 上で改行として描画される。
...
| ファイル | 変更種別 | 内容 |
|---|---|---|
| `src/renderer/renderer.js` | 削除 | `initEditor()` の `Editor` コンストラクタから `customHTMLRenderer: { hardBreak() {...} }` オプションを削除 |
| `src/renderer/renderer.js` | 変更 | `preprocessMarkdown()` の正規表現を `/ {2,}(\n)/g` → `'  $1'` から `/ {2,}\n/g` → `'<br>\n'` に変更 |
