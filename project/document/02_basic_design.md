...
| 1.2.0 | 2026-03-04 | CommonMark準拠要件対応。EditorコンポーネントにMarkdown前処理責務を追記。Toolbarにタスクリストボタンを追加。データフローにMarkdown読み込み前処理を追加（要件定義 v1.3.0 対応） |
...
| `Toolbar` | 右ペイン | 見出し、太字、リスト、**タスクリスト**等の書式設定ボタン群。エディタへのコマンド発行を行う。 | Toast UI Editor APIを使用 |
| `Editor` | 右ペイン | WYSIWYGエディタ本体。Markdown/HTMLの変換と表示、編集を受け持つ。**CommonMark準拠のための読み込み時Markdown前処理**（trailing-space保護等）もこのコンポーネントの責務とする。 | Toast UI Editor をラップ |
...
2. **ファイル読み込み（D&D）**:
   ...
   - **[v1.2.0追加] Markdown前処理**: `editor.setMarkdown()` 呼び出し前に、CommonMark準拠のレンダリングを保証するための前処理（trailing-space保護・ライブラリ差異吸収等）を`Editor`コンポーネント内で実施する。
3. **ファイル編集**:
